...
class Viewer {
  constructor({ container }) {
    ...
    Promise.all(bvhsLoaded).then((results) => {
      const controller = (this.controller = new Controller({ container: container, bvhs: results }));
...
class Controller extends GUI {
  constructor({ container, bvhs }) {
    super();

    // renderer
    const renderer = (this.renderer = new Renderer({ container: container, bvhs: bvhs }));
    ...
    //renderer.orbit.addEventListener("change", () => this.renderer.update(this.curPlayerMode, this.getMode(sceneConfs), undefined, undefined), false);
  ...
  update(playerMode, sceneMode, cameraMode, solverMode, selectMode, event) {
    ...
    this.renderer.update(playerMode, sceneMode, cameraMode, event);
  }
...
class Renderer extends THREE.WebGLRenderer {
  ...
  update(playerMode, sceneMode, cameraMode, event) {
    ...
    if (sceneMode === "lapScene") {
      this.updateRenderer(this.lapScene, camera, 0, width, height, raycaster);
    } else if (sceneMode == "sepScene") {
      this.updateRenderer(this.refScene, camera, 0, width / 2, height, raycaster);
      this.updateRenderer(this.cmpScene, camera, 0 + width / 2, width / 2, height, raycaster);
    }
  ...
  updateRenderer(scene, camera, left, width, height, raycaster) {
